...
router.get('/', async (req,res)=>{
    ...
    const data = await getInteraction('dislikes', field1, condition1, field2, condition2);
...
router.post('/', async(req,res)=>{
    ...
    const data = await create('dislikes', fields, object);
    res.status(200).json(data);
});
...
router.delete('/', async(req,res)=>{
    ...
    const condition1 = req.query.user_id;
    const condition2 = req.query.post_id;
    const data= await removeDoubleCondition('dislikes', field1, condition1, field2, condition2);
    res.status(200).json(data);
});
